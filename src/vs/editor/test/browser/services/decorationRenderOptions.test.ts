...
describe('Decoration Render Options', () => {
    ...
    expect(sheet.indexOf(`{background:url('https://github.com/microsoft/vscode/blob/main/resources/linux/code.png') no-repeat;background-size:contain;}`) >= 0).toBeTruthy()
    expect(sheet.indexOf(`{background-color:red;border-color:yellow;box-sizing: border-box;}`) >= 0).toBeTruthy()
	});
    ...
    expect(readStyleSheet(styleSheet).indexOf(`{background:url('data:image/svg+xml;base64,PHN2ZyB4b+') no-repeat;}`) > 0).toBeTruthy()
		s.removeDecorationType('example');

		function assertBackground(url1: string, url2: string) {
			const actual = readStyleSheet(styleSheet);
      expect(actual.indexOf(`{background:url('${url1}') no-repeat;}`) > 0
        || actual.indexOf(`{background:url('${url2}') no-repeat;}`) > 0).toBeTruthy()
		}
    ...
    expect(readStyleSheet(styleSheet).indexOf(`{background:url('http://test/pa%27th') no-repeat;}`) > 0).toBeTruthy()
		s.removeDecorationType('example');
	});
});
