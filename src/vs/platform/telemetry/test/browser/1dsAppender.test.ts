...
describe('AIAdapter', () => {
  ...
  test('property limits', () => {
    ...
    expect(reallyLongPropertyName.length > 150).toBe(true);
    ...
    expect(reallyLongPropertyValue.length > 8192).toBe(true);
    ...
    for (const prop in appInsightsMock.events[0].properties!) {
      expect(prop.length < 150).toBe(true);
      expect(appInsightsMock.events[0].properties![prop].length < 8192).toBe(true);
    }
  });
